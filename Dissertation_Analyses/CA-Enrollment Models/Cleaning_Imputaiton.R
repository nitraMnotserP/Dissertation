...
gddat.imp <- gddat %>%
  ...
  filter(
    replace_na(s4hscred, 0) > 0 | replace_na(s3hscredtype, 0) > 0
  ) %>%
...
imputation <- futuremice(data = dat.imp, m = 30, maxit = 10, parallelseed = 9204857, print = TRUE)

imps_long <- complete(imputation, include = FALSE, action = "long")

write.csv(imps_long, "CA_Enrollment_Imputed_Long.csv")
