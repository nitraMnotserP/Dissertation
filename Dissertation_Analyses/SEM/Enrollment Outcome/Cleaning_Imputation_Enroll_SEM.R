...
table(gddat$s4hscred)
table(gddat$s3hscredtype)
...
gddat.imp <- gddat %>%
  mutate(
    s3hscredtype = to_na_missing(s3hscredtype),
    s4hscred = to_na_missing(s4hscred),
    
    s3_hs_grad = case_when(
      s3hscredtype %in% c(
        "HIGH SCHOOL DIPLOMA",
        "GED OR OTHER HIGH SCHOOL EQUIVALENCY",
        "CERTIFICATE OF ATTENDANCE"
      ) ~ 1,
      is.na(s3hscredtype) ~ NA_real_,
      TRUE ~ 0
    ),
    
    s4_hs_grad = case_when(
      s4hscred %in% c(
        "YES, A HIGH SCHOOL DIPLOMA",
        "YES, A GED",
        "YES, ANOTHER HIGH SCHOOL EQUIVALENCY",
        "YES, CERT OF ATTENDANCE OR COMPLETION"
      ) ~ 1,
      is.na(s4hscred) ~ NA_real_,
      TRUE ~ 0
    ),
    
    hs_grad = coalesce(s4_hs_grad, s3_hs_grad),
    ...
    ##define outcome: s4evratndclg
    s4evratndclg = to_na_missing(s4evratndclg),  
    Enrolled = case_when(
      s4evratndclg == "YES" ~ 1,
      s4evratndclg == "NO" ~ 0
    ),
  ...
  select(
    stu_id, hs_grad, Locale:m1famsupport.2, s1frndgrades, s1frndschool:s1frndtalkm, w4w1w2w3stu
  )

table(gddat.imp$Enrolled, useNA = "ifany")
table(gddat.imp$hs_grad, useNA = "ifany")
##########################################################################################################################
...
dat.new <- cbind(gddat.imp, PBET_factor_scores, PSD_factor_scores, Peer_SC_factor_scores) %>% 
  select(stu_id:mathTach_expect, PerSchDisorder, PerBarEffTch, hs_grad,
...
sum(is.na(dat.new$Peer_Course_Taking))
sum(is.na(dat.new$Peer_Talk_Future))
sum(is.na(dat.new$Peer_Academic_Orient))
sum(is.na(dat.new$Peer_SC))
sum(is.na(dat.new$PerSchDisorder))
sum(is.na(dat.new$PerBarEffTch))


##Bind that back into the dataframe for imputaiton
dat.new <- cbind(dat.new, peer.sc) 

##########################################################################################################################
##################################Run Multiple Iputation Model via Mice PAckage###########################################
##########################################################################################################################

imputation <- mice::futuremice(data = dat.new, m = 10, maxit = 10, parallelseed = 9204857, print = TRUE)

imps_long <- mice::complete(imputation, include = FALSE, action = "long")

write.csv(imps_long, "CA_Enrolled_Imputed_Long_SEM.csv")

##Move to Enroll_SEM_DATA_PREP.R 
