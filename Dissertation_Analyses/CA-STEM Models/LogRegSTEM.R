...
##read in the data
imps_long <- read.csv("CA_STEM_Imputed_Long.csv")

##read in BRR weigths
weights <- read.csv("BRR_weights.csv", header = TRUE) %>% 
  select(stu_id:w4w1w2w3stu200)

imp_1$STEM_Major <- as.factor(imp_1$STEM_Major)
levels(imp_1$STEM_Major)
...
quantile(imp_1$SES, probs = c(0.33, 0.66))
...
##49.2          57.6
test %>%
  summarize(elec_bill = survey_quantile(Math_Ach,
                                        quantiles = c(0.33, 0.66)
  ))

quantile(imp_1$Math_Ach, probs = c(0.33, 0.66))
########################
######START HERE########
########################
...
imp_list <- lapply(imp_list, function(df){
  ...
  df %>% 
    mutate(
      ...
      ## Math groups: 50.37232 58.80380
      Low_math  = if_else(Math_Ach < 50.37232, 1, 0, missing = NA_real_),
      Mid_math  = if_else(Math_Ach >= 50.37232 & Math_Ach <= 58.80380, 1, 0, missing = NA_real_),
      High_math = if_else(Math_Ach > 58.80380, 1, 0, missing = NA_real_),
      ...
      ## SES groups: -0.1236  0.6000
      Low_SES  = if_else(SES < -0.1236, 1, 0, missing = NA_real_),
      Mid_SES  = if_else(SES >= -0.1236 & SES <= 0.6000, 1, 0, missing = NA_real_),
      High_SES = if_else(SES > 0.6000, 1, 0, missing = NA_real_),
...
mod1 <- with(hsls.svy,
             svyglm(STEM_Major ~ Black + Hispanic + Asian + Othrace + male + 
...
mod1.white <- with(hsls.svy.white,
                   svyglm(STEM_Major ~  male + high_math_course + rural + city + town + private + 
...
mod2.white <- with(hsls.svy.white,
                   svyglm(STEM_Major ~ male + high_math_course + rural + city + town + private + lowS_lowM + 
                            lowS_highM + midS_lowM + midS_highM + highS_lowM + highS_midM + 
                            highS_highM + school_climate + mathTach_expect + 
                            PerSchDisorder + PerBarEffTch,
                          family = quasibinomial("logit"), multicore = TRUE))
...
mod1.male <- with(hsls.svy.male,
                  svyglm(STEM_Major ~  Black + Hispanic + Asian + Othrace +
                           high_math_course + rural + city + town + private + lowS_midM + 
                           lowS_highM + midS_midM + midS_highM + highS_lowM + highS_midM + 
                           highS_highM + school_climate + mathTach_expect + 
                           PerSchDisorder + PerBarEffTch, 
...
write.csv(Full.sample.tab, "STEM_Full.csv")
write.csv(White.sample.tab, "STEM_White.csv")
write.csv(Black.sample.tab, "STEM_Black.csv")
write.csv(Hispanic.sample.tab, "STEM_Hispanic.csv")
write.csv(Male.sample.tab, "STEM_Male.csv")
write.csv(Female.sample.tab, "STEM_Female.csv")
...
write.csv(full.sample.pooled.ame, "STEM_Full_AME.csv")
write.csv(white.sample.pooled.ame, "STEM_White_AME.csv")
write.csv(black.sample.pooled.ame, "STEM_Black_AME.csv")
write.csv(hispanic.sample.pooled.ame, "STEM_Hispanic_AME.csv")
write.csv(male.sample.pooled.ame, "STEM_Male_AME.csv")
write.csv(female.sample.pooled.ame, "STEM_Female_AME.csv")
################################################################################################
