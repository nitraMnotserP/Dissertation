hsls.1 <- EdSurvey::readHSLS(path = "/Users/prestonmartin/Documents/Dissertation Analyses/HSLS/HSLS/2009", ##where you saved the data
...
gddat.imp <- gddat %>%
  mutate(
    ...
    # Enrolled: ⚠️ confirm the source column; using x3thimath looks like a typo.
    # If the correct field is e.g. x4enr_status, replace below accordingly.
    enrolled = to_na_missing(s4evratndclg),   # <-- likely needs a different source column
    enrolled = as.factor(enrolled),
    
    STEM_Major = x4rfdgmjstem,   # <-- likely needs a different source column
    STEM_Major = as.factor(x4rfdgmjstem),
    ...
    SES      = x1ses,
    Math_Ach = x1txmtscor,
    Math_Ach = if_else(Math_Ach == -8, NA_real_, Math_Ach),
  ...
  select(
    stu_id, Locale:m1famsupport.2, s1frndschool:s1frndtalkm, w4w1w2w3stu
  )

psych::describe(gddat.imp)
table(gddat.imp$STEM_Major)
##########################################################################################################################
...
fa_model.3  <- gddat.imp %>% 
  select(s1frndschool:s1frndtalkm) %>% 
  fa(nfactors = 3, scores = "regression", fm = "ml")
...
Peer_SC_factor_scores <- Peer_SC_factor_scores %>% 
  rename(
    ...
    Peer_Talk_Future = ML2,
    Peer_Academic_Orient = ML3
  )
#ML1 = s1frndtlkoth + s1frndtalks + s1frndtalkm -- Peer talk about course taking
#ML2 = s1frndtlkjob + s1frndtlkprb + s1frndtlkclg -- Peer Talk about Future
#ML3 = s1frndschool + s1frndclass + s1frndclg --Peer Academic Orientation


# Bind the scores to your dataset
dat.new <- cbind(gddat.imp, PBET_factor_scores, PSD_factor_scores, Peer_SC_factor_scores) %>% 
  select(stu_id:mathTach_expect, PerSchDisorder, PerBarEffTch, 
         Peer_Course_Taking:Peer_Academic_Orient, s1frndschool:s1frndtalkm, w4w1w2w3stu)

imputation <- futuremice(data = dat.new, m = 30, maxit = 10, parallelseed = 9204857, print = TRUE)

imps_long <- complete(imputation, include = FALSE, action = "long")

write.csv(imps_long, "CA_STEM_Imputed_Long.csv")
